pipeline {
    ...
    stages {
        ...
        stage('Sonarqube Analysis') {
            steps {
                        sh ''' $SCANNER_HOME/bin/sonar-scanner -Dsonar.url=http://3.7.254.184:9000/ -Dsonar.login=squ_0b16f660e5bc4514a3d8da45c5b9b896a6931f15 -Dsonar.projectName=shopping-cart \
        ...
        stage('Deploy to JFrog Artifactory') {
            ...
            steps {
                script {
                    try {
                        def server = Artifactory.newServer url: 'http://3.7.254.184:8082/artifactory/r123-1/', credentialsId: 'jfrog-cred'
                        def uploadSpec = """{
                            "files": [
                                {
                                    "pattern": "target/*.jar",
